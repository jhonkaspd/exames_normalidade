...
CMAP_SEMAFORO = ["#10B981", "#F59E0B", "#EF4444"]
...
def color_scale_semaforo(values, vmin=0, vmax=100):
    """Retorna lista de cores RGB interpoladas no semáforo verde→âmbar→vermelho."""
    import matplotlib.colors as mc
    import matplotlib as mpl
    cmap = mpl.colors.LinearSegmentedColormap.from_list("sem", CMAP_SEMAFORO)
...
st.markdown(
    ...
    f"Hospital Unimed · Março 2026 · <b>{titulo_setor}</b></p>",
    unsafe_allow_html=True,
)
...
with col_a:
    ...
    cores_e = color_scale_semaforo(exame_norm["Pct"])
...
with col_f:
    ...
    if len(long_pac):
        ...
        long_pac["Rep_Dia"]  = (long_pac["Reps"]  / long_pac["Dias"]).round(2)

        # Ordenação para melhor leitura
        long_pac = long_pac.sort_values("Ex_Dia", ascending=True)

        fig_ld = go.Figure()

        # 🔵 Barras — Exames por dia
        fig_ld.add_trace(go.Bar(
            y=long_pac["Atendimento"].astype(str),
            x=long_pac["Ex_Dia"],
            orientation="h",
            name="Exames/dia",
            marker_color="#3B82F6",
        ...
        fig_ld.add_trace(go.Scatter(
            ...
            marker=dict(size=8),
            xaxis="x2",
            hovertemplate="Rep/dia: %{x:.2f}<extra></extra>",
        ))

        # Layout com eixo duplo
        fig_ld.update_layout(
            title="Internações ≥7d — Intensidade Assistencial vs Desperdício",
            height=420,
...
st.markdown(
    ...
    "Lab Monitor · Hospital Unimed · Dados: ProntuárioMV/HU · Referência de custos: Custo Pardini"
    "</p>",
    unsafe_allow_html=True,
)
